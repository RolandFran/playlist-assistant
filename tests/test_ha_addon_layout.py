...
class HomeAssistantAddonLayoutTests(unittest.TestCase):
    def test_addon_build_context_contains_runtime_and_dependencies(self):
        ...
        self.assertIn('version: "0.1.35"', config)
        self.assertIn('panel_icon: mdi:play', config)
        self.assertNotIn('log_level: info', config)
        self.assertNotIn('log_level: "list(debug|info|warning|error)"', config)
        ...
        self.assertNotIn("bashio::addon.option", run_script)
        self.assertNotIn("spotify_client_id spotify_client_secret bridge_token", run_script)
        ...
        self.assertNotIn("configured_log_level=%s", service)
