...
class HomeAssistantAddonLayoutTests(unittest.TestCase):
    def test_addon_build_context_contains_runtime_and_dependencies(self):
        ...
        self.assertIn('version: "0.1.34"', config)
        ...
        for legacy_option in ("spotify_client_id", "spotify_client_secret", "bridge_token"):
            self.assertNotIn(legacy_option, config)
        run_script = (ADDON / "run.sh").read_text(encoding="utf-8")
        self.assertIn("bashio::addon.option", run_script)
        self.assertIn("spotify_client_id spotify_client_secret bridge_token", run_script)
        ...
        self.assertIn("PLAYLIST_ASSISTANT_SPOTIFY_PROXY", service)
