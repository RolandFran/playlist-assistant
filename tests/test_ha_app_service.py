...
class HomeAssistantServiceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.service = ServiceHost(paths=self.paths, options=AppOptions())

    def tearDown(self):
        self.directory.cleanup()

    def test_configuration_uses_the_home_assistant_proxy(self):
        self.assertEqual(
            spotify_environment(AppOptions(), self.paths),
    ...
    def test_schedule_change_notifies_the_production_integration(self):
        requests = []
        ...
        with patch.dict(os.environ, {"SUPERVISOR_TOKEN": "token"}, clear=True), patch(
            "ha_app.service.urllib.request.urlopen", urlopen
        ):
