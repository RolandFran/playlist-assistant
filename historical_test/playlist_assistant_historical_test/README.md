Playlist Assistant Historical Test brings music you already like back into rotation by building
...
reporting. The companion `playlist_assistant_historical_test` custom integration owns native
...
custom-repository flow, then install **Playlist Assistant Historical Test**. The package has
no `/config`, Node-RED, host-device, dashboard, automation, or integration
dependency.

## Home Assistant integration

Copy `custom_components/playlist_assistant_historical_test/` from this repository to
`/config/custom_components/playlist_assistant_historical_test/`, restart Home Assistant, then
add **Playlist Assistant Historical Test** from Settings → Devices & services. Enter the
add-on's internal URL (`http://playlist_assistant_historical_test:8108`) and the same
...
Spotify is linked only in the Playlist Assistant Historical Test Home Assistant integration.
...
- `/data/playlist_assistant_historical_test.db` is the historical-test database.
