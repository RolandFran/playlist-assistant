...
LOGGER = logging.getLogger("playlist_assistant_historical_test.ha_app")
DEFAULT_TICK_SECONDS = 60
DEFAULT_HEALTH_PORT = 8109
...
class ServiceHost:
    ...
    def tick(self) -> list[object]:
        ...
        try:
            ...
            request = urllib.request.Request("http://supervisor/core/api/playlist_assistant_historical_test/spotify", data=b'{"operation":"current_user"}', method="POST", headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"})
    ...
    def _notify_schedule_changed(self, config) -> None:
        ...
        request = urllib.request.Request("http://supervisor/core/api/services/playlist_assistant_historical_test/reconfigure_schedule", data=data,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Playlist Assistant Historical Test Home Assistant app service")
