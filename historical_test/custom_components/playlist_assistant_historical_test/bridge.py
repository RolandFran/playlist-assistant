...
ADDON_INGRESS_PORT = 8108
ADDON_NAME = "Playlist Assistant Historical Test"


class AddonDiscoveryError(RuntimeError):
    """The Supervisor could not identify the installed Playlist Assistant Historical Test app."""
...
async def async_discover_addon_base_url(session):
    ...
    if not token:
        raise AddonDiscoveryError("Supervisor token is unavailable; cannot discover the Playlist Assistant Historical Test add-on.")
    ...
    if len(matches) != 1:
        ...
        raise AddonDiscoveryError(
            "Expected exactly one Playlist Assistant Historical Test add-on; "
            f"found {len(matches)}. "
