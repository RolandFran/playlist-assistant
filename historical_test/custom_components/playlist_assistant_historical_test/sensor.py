...
class PlaylistStatus(_BridgeEntity):
    _attr_name = "Playlist Assistant Historical Test status"
    _attr_unique_id = "playlist_assistant_historical_test_status"
    @property
    def native_value(self): return self.coordinator.data.get("preview", {}).get("state", "idle")

class PlaylistConnection(_BridgeEntity):
    _attr_name = "Playlist Assistant Historical Test connection"
    _attr_unique_id = "playlist_assistant_historical_test_connection"
    @property
